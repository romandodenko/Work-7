<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta property="og:type" content="website">
  <meta property="og:title"
    content="Juego de acción con un mundo abierto y una nueva historia. Juego disponible en PS5. Demon’s Souls">
  <meta property="og:site_name"
    content="El juego ofrece gráficos mejorados y un tema musical pegadizo. El juego tendrá una mecánica de combate reelaborada y nuevas características con efectos especiales mejorados.">
  <meta property="og:url" content="oficial-clubes-de-juego.com">
  <meta property="og:image" content="oficial-clubes-de-juego.com/main.png">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title"
    content="Juego de acción con un mundo abierto y una nueva historia. Juego disponible en PS5. Demon’s Souls">
  <meta name="twitter:description"
    content="El juego ofrece gráficos mejorados y un tema musical pegadizo. El juego tendrá una mecánica de combate reelaborada y nuevas características con efectos especiales mejorados.">
  <meta name="twitter:site" content="oficial-clubes-de-juego.com">
  <meta name="twitter:image" content="oficial-clubes-de-juego.com/main.png">
  <meta name="description"
    content="El juego ofrece gráficos mejorados y un tema musical pegadizo. El juego tendrá una mecánica de combate reelaborada y nuevas características con efectos especiales mejorados.">
  <title>Juego de acción con un mundo abierto y una nueva historia. Juego disponible en PS5. Demon’s Souls</title>
  <link rel="shortcut icon" href="./img/favicon.png">
  <link rel="preload" href="./fonts/Karla-Medium.woff2" as="font" type="font/woff2" crossorigin>
  <link rel="preload" href="./fonts/Karla-Regular.woff2" as="font" type="font/woff2" crossorigin>
  <link rel="preload" href="./fonts/Tomorrow-Bold.woff2" as="font" type="font/woff2" crossorigin>
  <link rel="stylesheet" href="./css/main-min.css">
  <script defer src="./js/main.js"></script>
</head>

<body>
  <div class="page">
    <!-- start header -->
    <?php
    include 'header.php';
    ?>
    <!-- end header -->
    <main>
      <!-- start game -->
      <section class="game">
        <div class="container">
          <h1 class="game__title tt-3">
            Demon’s Souls
          </h1>
          <div class="game__content">
            <div class="game__left">
              <div class="game__ps">
                <h2 class="game__subtitle tx-2">
                  Disponible en
                </h2>
                <ul class="game__list">
                  <li class="game__item tt-4">
                    PS5
                  </li>
                  <li class="game__item tt-4">
                    PC
                  </li>
                  <li class="game__item tt-4">
                    Xbox Series X
                  </li>
                </ul>
              </div>
              <div class="game__image game__image_1">
                <picture>
                  <source media="(min-width: 320px)" srcset="./img/game/game-1.webp" type="image/webp">
                  <img src="./img/game/game-1.jpg" loading="lazy" width="300" height="300" alt="Image">
                </picture>
              </div>
            </div>
            <div class="game__right">
              <div class="game__image game__image_2">
                <picture>
                  <source media="(min-width: 320px)" srcset="./img/game/game-1-1.webp" type="image/webp">
                  <img src="./img/game/game-1-1.jpg" loading="lazy" width="300" height="300" alt="Image">
                </picture>
              </div>
              <div class="game__descr">
                <p class="game__text tx-1">
                  Demon's Souls es un juego de rol de acción desarrollado por FromSoftware y lanzado en 2009
                  exclusivamente para la consola PlayStation 3.
                </p>
                <p class="game__text tx-1">
                  El jugador controla un personaje ubicado en el mundo de Swampstone. Este mundo está bajo amenaza
                  debido a una maldición que convierte a los habitantes en demonios. El jugador debe viajar a diferentes
                  lugares, derrotar enemigos y recolectar elementos para volverse más fuerte y proteger Swampstone.
                </p>
                <p class="game__text tx-1">
                  La jugabilidad en Demon's Souls es bastante compleja y requiere habilidad y estrategia por parte del
                  jugador. Por ejemplo, el juego proporciona pocas pistas o ninguna pista y el jugador debe explorar los
                  niveles por su cuenta y encontrar formas de derrotar a los enemigos.
                </p>
                <p class="game__text tx-1">
                  El fracaso puede resultar en la pérdida de todas las almas acumuladas, la moneda del juego utilizada
                  para subir de nivel al personaje y adquirir nuevos equipos. Sin embargo, aunque el juego es conocido
                  por su dificultad, ha sido elogiado por su jugabilidad única y su convincente historia.
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>
      <!-- end game -->

      <!-- start games -->
      <section class="games">
        <div class="container">
          <h2 class="games__title tt-2">
            Otros juegos
          </h2>
          <ul class="games__list">
            <li class="games__item">
              <div class="games__image">
                <a href="game-2.php">
                  <picture>
                    <source media="(min-width: 320px)" srcset="./img/game/game-2.webp" type="image/webp">
                    <img src="./img/game/game-2.jpg" loading="lazy" width="300" height="300" alt="Image">
                  </picture>
                </a>
              </div>
              <div class="games__descr">
                <a class="games__name tx-2" href="game-2.php">
                  Elden Ring
                </a>
                <p class="games__text tx-1">
                  Elden Ring es un nuevo juego de rol de mundo completamente abierto del famoso estudio japonés
                  FromSoftware, desarrollado por Hidetaka Miyazaki con la participación del famoso escritor
                  estadounidense George Martin.
                </p>
                <a class="games__see but-3" href="game-2.php">
                  Más
                </a>
              </div>
            </li>
            <li class="games__item">
              <div class="games__image">
                <a href="game-3.php">
                  <picture>
                    <source media="(min-width: 320px)" srcset="./img/game/game-3.webp" type="image/webp">
                    <img src="./img/game/game-3.jpg" loading="lazy" width="300" height="300" alt="Image">
                  </picture>
                </a>
              </div>
              <div class="games__descr">
                <a class="games__name tx-2" href="game-3.php">
                  Dead Space
                </a>
                <p class="games__text tx-1">
                  Dead Space Remake es una nueva versión completa del juego de terror espacial de culto en la última
                  versión del motor gráfico Frostbite.
                  Más.
                </p>
                <a class="games__see but-3" href="game-3.php">
                  Más
                </a>
              </div>
            </li>
          </ul>
        </div>
      </section>
      <!-- end games -->
    </main>
    <!-- start footer -->
    <?php
    include 'footer.php';
    ?>
    <!-- end footer -->
  </div>
</body>

</html>